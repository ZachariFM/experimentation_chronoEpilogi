...
for mts_description in mts_list:
    ...
    data.columns = list(map(str, data.columns))  # column names as str

    tuning_data = data.iloc[:int(len(data)*train_test_split)]
    holdout_data = data.iloc[int(len(data)*train_test_split):]
    ...
    list_fs = ["NoSelection", "CausalPairs","SingleChronoEpilogi", "GroupLasso"]
    ...
    for fs_name in list_fs:
        ...
        for forecaster_name in list_forecasters:
            ...
            # launch
            result = test_single.test_single_solution(
                        results_directory+dataset_name,
                        tuning_data,
                        holdout_data,
                        ...
                        es_config,
                        target, 
                        causal_ground_truth=gt)
                
            if result is not None and isinstance(result, tuple) and len(result) == 2:
                df_params, df_results = result
                test_single.save_testing_results(results_directory+dataset_name+"_test" ,df_params, df_results)
            else:
                print("Warning: test_single_solution did not return expected results for", fs_name, forecaster_name)
